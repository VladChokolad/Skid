...
func (s *Storage) CreatePurchase(purchase objects.Purchase) (int, error) {
	...
	err := s.db.QueryRow(
		`INSERT INTO 
		purchases (party_id, buyer_id, name, description, purchase_icon_id, 
		price, split_type, date_of_purchase) 
...
func (s *Storage) UpdatePurchase(Purchase objects.Purchase) error {
	...
	result, err := s.db.Exec(
		`UPDATE purchases 
		SET buyer_id = $1, name = $2, description = $3, purchase_icon_id = $4, price = $5, split_type = $6, date_of_purchase = $7 
		WHERE id = $8`,
